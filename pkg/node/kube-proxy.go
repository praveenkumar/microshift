...
func NewKubeProxyServer(cfg *config.Config) *ProxyOptions {
	s := &ProxyOptions{}
	s.configure(cfg)
	return s
}
...
func (s *ProxyOptions) configure(cfg *config.Config) {
	...
	opts.ConfigFile = config.DataDir + "/resources/kube-proxy/config/config.yaml"
	opts.Complete(cmd.Flags())
	s.options = opts
	cmd.SetArgs(args)
...
func (s *ProxyOptions) writeConfig(cfg *config.Config) error {
	data := []byte(`
apiVersion: kubeproxy.config.k8s.io/v1alpha1
kind: KubeProxyConfiguration
clientConnection:
  kubeconfig: ` + cfg.KubeConfigPath(config.KubeProxy) + `
hostnameOverride: ` + cfg.Node.HostnameOverride + `
...
featureGates:
   AllAlpha: false`)
	...
	path := filepath.Join(config.DataDir, "resources", "kube-proxy", "config", "config.yaml")
...
func (s *ProxyOptions) Run(ctx context.Context, ready chan<- struct{}, stopped chan<- struct{}) error {
	...
	// run readiness check
	go func() {
		healthcheckStatus := util.RetryInsecureGet(ctx, "http://127.0.0.1:10256/healthz")
		if healthcheckStatus != 200 {
			klog.Fatalf("%s healthcheck failed due to kube-scheduler failure to start", s.Name())
